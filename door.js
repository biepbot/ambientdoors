  ...
  libWrapper.register(
    ...
    async function (wrapped, event) {
      ...
      if (this.wall.document.ds === CONST.WALL_DOOR_STATES.LOCKED) {
        ...
        const eventLockJingle = await onDoorMouseDown.call(this, event);
...
async function onDoorMouseDown(event) {
  const doorData =
    this.wall.document.flags.ambientdoors?.doorData || defaultDoorData();
  const playpath =
    doorData.lockJinglePath === "DefaultSound"
      ? game.settings.get(mod, "lockedDoorJinglePathDefault")
      : doorData.lockJinglePath;
  const playVolume = doorData.lockJingleLevel;

  if (playpath != "" && playpath != null) {
    await playSoundOnToken(
      event.center,
      playpath,
      playVolume,
      doorData.soundRadius
    );
  }
...
Hooks.on("preUpdateWall", async (object, updateData, diff, userID) => {
  if (object.door == 0 || updateData.ds == null) {
    //Exit early if not a door OR door state not updating
    return;
  }

  const sneakyDoorBinding = game.keybindings.bindings.get(
    `${mod}.sneakyDoor`
  )[0];
  if (
    sneakyDoorBinding &&
    game.users.find((x) => x._id === userID).role >=
      game.settings.get(mod, "stealthDoor") &&
    game.keyboard.downKeys.has(sneakyDoorBinding.key)
  ) {
    // Exit if Sneaky Door Opening Mode
    return;
  }

  let doorData;
  try {
    doorData = object.flags.ambientdoors.doorData;
  } catch (err) {
    //If data is set us that
    doorData = defaultDoorData();
  ...
  if (object.ds == 2) {
    // Door Unlocking
    playpath =
      doorData.unlockPath === "DefaultSound"
        ? game.settings.get(mod, "unlockDoorPathDefault")
        : doorData.unlockPath;
    playVolume = doorData.unlockLevel;
  } else if (updateData.ds == 0) {
    //Door Close
    playpath =
      doorData.closePath === "DefaultSound"
        ? game.settings.get(mod, "closeDoorPathDefault")
        : doorData.closePath;
    playVolume = doorData.closeLevel;
  } else if (updateData.ds == 1) {
    //Door Open
    playpath =
      doorData.openPath === "DefaultSound"
        ? game.settings.get(mod, "openDoorPathDefault")
        : doorData.openPath;
    playVolume = doorData.openLevel;
  } else if (updateData.ds == 2) {
    // Door Lock
    playpath =
      doorData.lockPath === "DefaultSound"
        ? game.settings.get(mod, "lockDoorPathDefault")
        : doorData.lockPath;
    playVolume = doorData.lockLevel;
  }

  if (playpath != "" && playpath != null) {
    await playSoundOnToken(object, playpath, playVolume, doorData.soundRadius);
  }
});

Hooks.on("renderWallConfig", (app, html, data) => {
  if (data.object.door == 0) {
    // if it's not a door don't worry
    return;
  }
  ...
  const message = `
    ...
    <div class="form-group">
		...
		<div class="form-fields">
			...
			<input class="sound" type="text" name="flags.ambientdoors.doorData.closePath" value="${
        closeFlag ? closeFlag : ``
      }" placeholder="Door Close Sound Path" data-dtype="String" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="form-fields">
			<input type="range" name="flags.ambientdoors.doorData.closeLevel" value=${
        thisDoor.closeLevel
      }  min=0 max=2 step=0.05 data-dtype="Number">
    ...
    <div class="form-group">
		...
		<div class="form-fields">
			...
			<input class="sound" type="text" name="flags.ambientdoors.doorData.openPath" value="${
        openFlag ? openFlag : ``
      }" placeholder="Door Open Sound Path" data-dtype="String" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="form-fields">
			<input type="range" name="flags.ambientdoors.doorData.openLevel" value=${
        thisDoor.openLevel
      }  min=0 max=2 step=0.05 data-dtype="Number">
	...
	<div class="form-group">
		...
		<div class="form-fields">
			...
			<input class="sound" type="text" name="flags.ambientdoors.doorData.lockPath" value="${
        lockFlag ? lockFlag : ``
      }" placeholder="Door Lock Sound Path" data-dtype="String" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="form-fields">
			<input type="range" name="flags.ambientdoors.doorData.lockLevel" value=${
        thisDoor.lockLevel
      }  min=0 max=2 step=0.05 data-dtype="Number">
	...
	<div class="form-group">
		...
		<div class="form-fields">
			...
			<input class="sound" type="text" name="flags.ambientdoors.doorData.unlockPath" value="${
        unLockFlag ? unLockFlag : ``
      }" placeholder="Door Unlock Sound Path" data-dtype="String" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="form-fields">
			<input type="range" name="flags.ambientdoors.doorData.unlockLevel" value=${
        thisDoor.unlockLevel
      }  min=0 max=2 step=0.05 data-dtype="Number">
    ...
    <div class="form-group">
		...
		<div class="form-fields">
			...
			<input class="sound" type="text" name="flags.ambientdoors.doorData.lockJinglePath" value="${
        lockJingleFlag ? lockJingleFlag : ``
      }" placeholder="Locked Door Jingle Sound Path" data-dtype="String" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="form-fields">
			<input type="range" name="flags.ambientdoors.doorData.lockJingleLevel" value=${
        thisDoor.lockJingleLevel
      }  min=0 max=2 step=0.05 data-dtype="Number">
	...
	<div class="form-group">
      ...
      }  min=0.5 max=100 step=0.5 data-dtype="Number">
			<span class="range-value">${thisDoor.soundRadius}</span>
		</div>
	</div>
	`;
